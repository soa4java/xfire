...
public abstract class GroupChatCrossDomainHelper {
    ...
    public static Map<String, UserTicket> getPidAndDomainMapForOnline(Set<Object> pids) {

    	Map<String, UserTicket> map = DomainNodeJidCacheUtils.multiFetch(pids);
    	Map<String, UserTicket> onlineMap = Maps.newHashMap(); 
    	
    	if(MapUtils.isEmpty(map)){
    		return onlineMap;
    	}
    	
        for(Map.Entry<String, UserTicket> entry:map.entrySet()){
        	UserTicket userTicket = entry.getValue();
        	if(userTicket == null){
        		continue;
        	}
        	String status = userTicket.getSs() ;
        	if(StateEnum.AVAIL.code().equalsIgnoreCase(status)  || StateEnum.AWAY.code().equalsIgnoreCase(status) ){
